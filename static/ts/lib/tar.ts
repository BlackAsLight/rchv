...
export class TarStream {
	...
	constructor() {
		const { readable, writable } = new TransformStream<TarStreamFile | TarStreamDir, TarStreamFile | TarStreamDir>()
		const gen = (async function* () {
			...
			for await (const chunk of readable) {
					...
					typeof chunk.pathname === 'string'
						...
						: (function () {
							if ('size' in chunk === (chunk.pathname[1].slice(-1)[0] === 47)) {
								throw new Error(
									`Pre-parsed pathname for ${'size' in chunk ? 'directory' : 'file'
				...
				header.set(
					encoder.encode(
						(chunk.options?.mode ?? (typeflag === '5' ? '755' : '644')).padStart(6, '0') +
						' \0' + // mode
						(chunk.options?.uid ?? '').padStart(6, '0') +
						' \0' + // uid
						(chunk.options?.gid ?? '').padStart(6, '0') +
						' \0' + // gid
						('size' in chunk ? chunk.size.toString(8) : '').padStart(sizeExtension ? 12 : 11, '0') +
						(sizeExtension ? '' : ' ') + // size
						(chunk.options?.mtime?.toString(8) ?? Math.floor(new Date().getTime() / 1000).toString(8)).padStart(
							11,
							'0',
						) +
						' ' + // mtime
				...
				if ('size' in chunk) {
					...
					if (chunk.size !== size) {
						throw new Error('Invalid Tarball! Provided size did not match bytes read from iterable.')
					}
					if (chunk.size % 512) {
...
export function parsePathname(pathname: string, isDirectory = false): [Uint8Array, Uint8Array] {
	pathname = pathname
		.split('/')
		.filter(x => x)
		.join('/')
		...
		this.#readable = new ReadableStream<TarStreamEntry>({
			...
			async pull(controller) {
				...
				const decoder = new TextDecoder()
				{
					...
					if (checksum.reduce((x, y) => x + y) !== parseInt(decoder.decode(value.slice(148, 156 - 2)), 8)) {
						return controller.error('Invalid Tarball. Header failed to pass checksum.')
					}
				}
				header = {
				...
				if ([117, 115, 116, 97, 114, 0, 48, 48].every((byte, i) => value[i + 257] === byte)) {
					header = {
						...
						devmajor: decoder.decode(value.slice(329, 337)).replaceAll('\0', ''),
						devminor: decoder.decode(value.slice(337, 345)).replaceAll('\0', ''),
